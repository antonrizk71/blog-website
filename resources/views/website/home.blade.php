...
<li class="nav-item dropdown">
    ...
    <ul class="dropdown-menu overflow-hidden" id="dropdownMenu">
        ...
        <form action="{{ route('articles.category', $category->id) }}" method="GET">
...
@section('Articles')
<section class=" container  gap-4 d-flex align-content-center justify-content-center flex-wrap ">
    @foreach($articles as $art)
    @if($loop->iteration % 2 == 0)
    <div class="card mt-5 mb-1 col-lg-3 col-md-8 col-sm-12 ">
        @else
        <div class="card mt-5 mb-1 col-lg-4 col-md-8 col-sm-12">
            @endif
            ...
            <div class="card-body">
                <div class="mt-3 d-flex justify-content-between ">
                    ...
                    <div class="d-flex gap-3">
                        ...
                        <div>
                            <button type="submit" style="background: none; border: none; cursor: pointer;"
                                data-bs-toggle="modal" data-bs-target="#myModal{{$art->id}}">
                                <i class="fa-regular fa-pen-to-square"></i>
                            </button>
                        </div>
                        @endif
                        <div class="modal" id="myModal{{$art->id}}" tabindex="-1" aria-labelledby="myModalLabel"
                            aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{route('articles.update',$art->id)}}" method="POST"
                                            enctype="multipart/form-data" class=" w-75 container  mt-5">
                                            @csrf
                                            @method('put')
                                            <div class="mb-3">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <textarea class="form-control" name="content" id="" cols="30"
                                                    rows="3">{{$art->content}}</textarea>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">Type</label>
                                                <select class="form-control" name="type">
                                                    @foreach($categories as $category)
                                                    <option value="{{$category->id}}" {{$art->type==$category->id?
                                            ...
                                            <button type="submit"
                                                class="btn btn-primary form-control mb-3 ">Save</button>
                                        </form>
            ...
            <ul class="list-group list-group-flush">
                {{-- <li class="list-group-item">Created At : {{
                    \Carbon\Carbon::parse($art->created_at)->format('Y-m-d') }} </li> --}}
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center ">
                    <div class="d-flex gap-1">
                       ...
                       <a class="text-decoration-none" href="{{route('user.articles',$art->admin->id)}}">{{$art->admin->name}}</a>
                    </div>
...
<div class="container d-flex mt-3 mb-5 justify-content-center">
    <div class="mb-5">
        {{$articles->links() }}
